...
var disabledInfoLogger = (*noopInfoLogger)(nil)

func (l *noopInfoLogger) Info(msg string, fields ...Field)               {}
func (l *noopInfoLogger) Infof(format string, v ...interface{})          {}
func (l *noopInfoLogger) Infow(msg string, keysAndValues ...interface{}) {}
...
func handleFields(l *zap.Logger, args []interface{}, additional ...zap.Field) []zap.Field {
	...
	if len(args) == 0 {
		// first-return if we have no sugared fields.
		return additional
	}
	...
	for i := 0; i < len(args); {
		...
		if _, ok := args[i].(zap.Field); ok {
			l.DPanic("strongly-typed Zap Field passed to logr", zap.Any("zap filed", args[i]))
...
func New(opts *Options) *zapLogger {
	...
	// when output to local path, with color if forbidden
